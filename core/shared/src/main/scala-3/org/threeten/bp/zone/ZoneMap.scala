...
private[bp] class ZoneMap[K: ClassTag: Ordering, V] private[bp] (var map: immutable.TreeMap[K, V])
...
object ZoneMap:
  def apply[K: ClassTag: Ordering, V](map: immutable.TreeMap[K, V]): java.util.NavigableMap[K, V] =
    new ZoneMap[K, V](map)
